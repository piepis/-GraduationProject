...
def RecognizePlateJson(image):
    ...
    for j,plate in enumerate(images):
        plate,rect,origin_plate =plate # 车牌，原图中车牌比例，近似车牌
...
def SimpleRecognizePlateByE2E(image):
    ...
    for j,plate in enumerate(images):
        ...
        res,confidence = e2e.recognizeOne(origin_plate) #输出预测的车牌号 以及 预测的概率
        print("res",res)
...
def SimpleRecognizePlate(image):
    t0 = time.time()
    images = detect.detectPlateRough(image,image.shape[0],top_bottom_padding_rate=0.1) #得到是车牌号的图片的 长宽高，原图，以及
    res_set = []
    for j,plate in enumerate(images):
        plate, rect, origin_plate  =plate
        # plate = cv2.cvtColor(plate, cv2.COLOR_RGB2GRAY)
        plate  =cv2.resize(plate,(136,36*2)) #将图片放大一些
        # cv2.imwrite('nono7.jpg',plate)
        t1 = time.time()
        ...
        image_rgb = fm.findContoursAndDrawBoundingBox(plate) #矫正过 后的图像
        #  cv2.imwrite('nono9.jpg',image_rgb)
        image_rgb = fv.finemappingVertical(image_rgb) #对矫正后的图像进行预测，然后放大一下
        cv2.imwrite('nono10.jpg',image_rgb)
        cache.verticalMappingToFolder(image_rgb) #存储可能是车牌号的到文件夹里，作为数据储备
        print("e2e:", e2e.recognizeOne(image_rgb))
        image_gray = cv2.cvtColor(image_rgb,cv2.COLOR_RGB2GRAY) #分割的图片去灰度
        ...
        if len(val)==3:
            ...
            #cv2.imwrite('nono14.jpg',image)
            if confidence/7>0.7:
                image =  drawRectBox(image,rect,res) #在原图像上加个输出红框框
                #cv2.imwrite('nono15.jpg', image)
                res_set.append(res)
