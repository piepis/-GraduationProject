...
model.load_weights("./model/plate_type.h5")  #初始化一个完全相同的模型
model.save("./model/plate_type.h5")  #model.save(filepath)将Keras模型和权重保存在一个HDF5文件中
def SimplePredict(image):
    image = cv2.resize(image, (34, 9))  #将车牌号缩小
    # cv2.imwrite('nono8.jpg',image)
