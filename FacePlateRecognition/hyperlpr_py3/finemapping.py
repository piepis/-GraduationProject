...
def findContoursAndDrawBoundingBox(image_rgb):
    ...
    for k in np.linspace(-50, 0, 15):
        ...
        binary_niblack = cv2.adaptiveThreshold(gray_image,255,cv2.ADAPTIVE_THRESH_MEAN_C,cv2.THRESH_BINARY,17,k)  #车牌号被阈值去掉成为 黑白的
        cv2.imwrite("nono4.jpg",binary_niblack)
        # cv2.waitKey(0)
    ...
    image,M = deskew.fastDeskew(image) # 对黑白图像矫正后的图像
    cv2.imwrite("nono5.jpg", image)
    return image
